...
function ZoneDetailCtrl($scope, $compile, $location, Restangular, server, zone) {
  ...
  $scope.export = function() {
    var output = "# Zone "+$scope.zone.name+" - Generated by pdnscontrol on " + moment().toString() + "\n";
    _.each($scope.zone.records, function(record) {
      var content = record.content;
      if (typesWithPriority.indexOf(record.type) != -1) {
        content = record.priority + " " + content;
      }
      output += record.name + ".\t" + record.ttl + "\tIN\t" + record.type + "\t" + content + "\n";
    });
    saveAs(new Blob([output], {type: "text/plain;charset="+document.characterSet}), $scope.zone.name+".zone");
  };
